...
int main(void) {
	...
	pthread_t sensor_thread, button_thread, reportero;
	pthread_attr_t attr;
	...
	pthread_create(&sensor_thread, &attr, sensor_monitor, &results);
...
void* btn_monitor(void* arg) {
	...
	struct shared_data *results = (struct shared_data *) arg;
	...
	for (index = 0; index < MAX_ITERATIONS; index++) {
		...
		if (button_pressed >= 0) {
			pthread_mutex_lock(&results->mutex);
			printf("Inside BUTTON MUTEX\n");
			results->button_pressed = "Btn Press!";
			pthread_mutex_unlock(&results->mutex);
...
void* reader(void *arg) {
	...
	for (index = 0; index < MAX_ITERATIONS_COLOR; index++) {
		pthread_mutex_lock(&results->mutex);
		ev3_text_lcd_large(0, 30, results->button_pressed);
		ev3_text_lcd_large(0, 60, results->color_sensor);
		ev3_text_lcd_large(0, 100, results->op_mode);
		pthread_mutex_unlock(&results->mutex);
