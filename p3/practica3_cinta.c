...
#define RW							2		// Radius of a wheel in cms
#define MOTOR_SPEED					169		// units: deg/seg
#define MOTOR_RUNTIME				1500	// units: ms
...
int main (void) {
    ...
    distance = conveyor_belt(belt_motor, MOTOR_SPEED, MOTOR_RUNTIME);
    printf ("Distance traveled %d (cms)\n", distance);
...
int conveyor_belt(ev3_motor_ptr belt_motor, int speed, int execution_time) {

	// Motor configuration
	ev3_stop_command_motor_by_name (belt_motor, STOP_MODE_STRING[BRAKE]);
	ev3_set_speed_sp (belt_motor, speed);  // Power to the engine
	ev3_set_position (belt_motor, 0);
	int distance;

	for(int i = 1; i < 4; i++){
		printf ("Start position %d (degrees) on hop # %d \n", ev3_get_position (belt_motor), i);
		ev3_set_time_sp (belt_motor, execution_time);
		...
		distance = (ev3_get_position (belt_motor) * PI/180) * RW;
	}
